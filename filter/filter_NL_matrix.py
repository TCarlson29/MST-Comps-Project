...
filtered_NL_data = 'filtered_NL_matrices'

# creates the folder if it doesn't exist
if not os.path.exists(filtered_NL_data):
    os.makedirs(filtered_NL_data)
...
if NL_csv_files:
    universal_brain_parts = read_first_column(NL_csv_files[0]) # gets the first column of the first csv file

    for file in NL_csv_files[1:]: # loops through all files until the end - want to start at 1 because of line above
    ...
    for file in NL_csv_files:
        ...
        output_file = os.path.join(filtered_NL_data, f"filtered_{os.path.basename(file)}") # allows the name change of csv file to be filtered

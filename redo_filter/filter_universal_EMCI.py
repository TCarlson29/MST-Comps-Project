...
# creates the folder name for the universal brain regions in the EMCI matrices
universal_regions_EMCI_matrices = 'EMCI_universal_regions_matrices'

# physically creates the folder if it doesn't exist
if not os.path.exists(universal_regions_EMCI_matrices):
    os.makedirs(universal_regions_EMCI_matrices)

# baseline set for the universal regions in the EMCI matrices
...
# walks through each EMCI file and removes the uncommon brain regions in all matrices.
if EMCI_csv_files:
    universal_brain_regions = read_first_column(EMCI_csv_files[0]) # gets the first column of the first csv file

    # loops through all files and removes the uncommon regions of the brain - want to start at 1 because of line above
    for file in EMCI_csv_files[1:]: 
    ...
    for file in EMCI_csv_files:
        ...
        output_file = os.path.join(universal_regions_EMCI_matrices, f"universal_regions_{os.path.basename(file)}")
